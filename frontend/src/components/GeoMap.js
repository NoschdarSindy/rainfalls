...
const GeoMap = forwardRef(
  ({ intervalViewId, filteredEvents, ...props }, ref) => {
    // (props, ref) => {     ({ intervalViewId, filteredEvents }) {
    // Tooltip für Events
...
SI: ${properties.si}<br>
    ...
    // "Expose" the overviewToDetail method to parent comonent
    ...
    const leafletMap = (
      <VisLeafletMap
        ref={mapRef}
        className={
          "overview-map fadable position-absolute overview-map-" +
          intervalViewId
        }
